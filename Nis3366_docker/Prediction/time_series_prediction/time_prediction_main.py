...
def Comb_plot_prediction(history2, combined_pred):
    """
    绘制历史数据与融合预测结果的对比图
    参数：
    ...
    xlabels[xticks.index(0)] = 'now'  # 将0点标记为now
    ...
    # 添加图表标题
    plt.title('Hotness Trend Prediction', fontsize=14, pad=20)
...
def plot_predictions(history, arima_pred, lstm_pred, ets_pred):
    """生成三模型预测对比图（单图合并版）"""
    ...
    ax.set_xticklabels(['-4', '-3', '-2', '-1', 'now', '1', '2', '3', '4', '5'])
    ...
    # 设置标题
    ax.set_title('Multi-Model Time Series Prediction', fontsize=14, pad=20)
...
def main(input_sequence=None):
    ...
    if input_sequence is None:
        input_seq = input("请输入待预测序列（逗号分隔，至少30个值）: ")
    else:
    ...
    try:
        ...
        # 结果可视化
        plot_predictions(history, arima_pred[:5], lstm_pred[:5], ets_pred[:5])
        ...
        history2 = sequence[-10:]  # 最后5个历史值
        Comb_plot_prediction(history2, combined_pred)    
