...
def generate_test_sequence(min_len: int = 10, max_len: int = 30) -> List[float]:
    """
    ...
    # 阶段划分（边界保护）
    phase1_end = max(3, int(length * 0.2))       # 上升期至少3天
    phase2_end = min(length-5, phase1_end + int(length * 0.6))  # 保留至少5天衰退期
...
def main():
    ...
    for i in range(1, num_records+1):
        try:
            seq = generate_test_sequence(min_len=10, max_len=max_columns-10)
            # 最终校验
            if len(seq) == 0:
